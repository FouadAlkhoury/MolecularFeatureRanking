...
report_graph_level_importance_raw = 'data/real/split/importance_nn_' + dataset + '.csv'
report_graph_level_ranking_raw = 'data/real/split/ranking_nn_' + dataset + '.csv'
report_graph_level_importance_agg = 'data/real/split/importance_agg_nn_' + dataset + '.csv'
report_graph_level_ranking_agg = 'data/real/split/ranking_agg_nn_' + dataset + '.csv'
...
for i in range(len(X)):
    ...
    for j in range(10):
        ...
        exp = explainer.explain_instance(X_train[0], model.predict_proba, num_features=X_train.shape[1])

        local_weights = np.zeros(X_train.shape[1])
        for idx, weight in exp.as_list():
            feat_idx = int(idx.replace("feat_", "").strip())
            local_weights[feat_idx] = abs(weight)

        feature_importances.append(local_weights)
        print(local_weights)
...
aggregated_importance = np.mean(np.vstack(feature_importances), axis=0)
arr_ordered_agg = np.argsort(aggregated_importance)[::-1]

writeToReport(report_graph_level_importance_agg, graphs[i][0] + ',' + list_to_str(list(aggregated_importance)))
...
writeToReport(time_file, 'NN,' + dataset + ', ' + str(diff_time))
